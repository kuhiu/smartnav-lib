...
FuzzyControlSystem FuzzyControlSystem::parse(nlohmann::json fuzzy_control_system_json) {
  ...
  // Parse outputs
  for (const auto &output : fuzzy_control_system_json.at(__OUTPUTS_KEY)) {
    try {
  ...
  // Parse rules
  for (const auto &rule : fuzzy_control_system_json.at(__RULES_KEY)) {
    try {
